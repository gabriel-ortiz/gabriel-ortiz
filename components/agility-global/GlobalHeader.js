...
const GlobalHeader = (props) => {
	...
	//console.log(globalHeaderProps);

	//contentItem is the site name and the logo, etc
	const globalHeaderItem = globalHeaderProps.contentItem;
	//header site name from contentItem
	let siteName = globalHeaderItem?.fields.siteName || 'Agility Starter 2020';
	let logo = globalHeaderItem?.fields.logo || null;
	...
	const links = [
		<NavLinks key={1} style={linkAnimation}>
			{globalHeaderProps.links.map((link) => (
				<Link key={link.path} href={`/[...slug]`} as={link.path}>
					<NavLink>{link.text}</NavLink>
				</Link>
			))}
			{/* Optionally show the primary cta field */}
			{globalHeaderItem.fields.primaryCTA && (
				<PrimaryLink
					href={globalHeaderItem.fields.primaryCTA.href}
					target={globalHeaderItem.fields.primaryCTA.target}>
					{globalHeaderItem.fields.primaryCTA.text}
				</PrimaryLink>
			)}
		</NavLinks>,
	];

	return <Header {...{ links, logoLink }} />;
};
...
GlobalHeader.getCustomInitialProps = async function (props) {
	...
	try {
		...
		//console.log('contentItemList', contentItemList);
