...
PostsListing.getCustomInitialProps = async function ({
    agility,
    channelName,
    languageCode,
}) {
    ...
    try {
        ...
        let tags = await api.getContentList({
            referenceName: 'tags',
            languageCode,
        });
        ...
        console.log('dynamicUrls', dynamicUrls);
        ...
        let posts = rawPosts.map((post) => {
            ...
            const description = truncate(post.fields.content, {
                length: 160,
                decodeEntities: true,
                stripTags: true,
